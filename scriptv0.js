...
groupHeaders.forEach(header => {
    header.addEventListener('click', () => {
        // Toggle class on header (for arrow rotation)
        header.classList.toggle('collapsed');
        
        // Toggle class on the content div (to hide/show)
...
collapseAllBtn.addEventListener('click', () => {
    ...
    // 1. Rotate all arrows (by adding the class to headers)
    headers.forEach(header => {
        header.classList.add('collapsed');
    });

    // 2. Hide all content (by adding the class to content divs)
